...
`POST /transactions`, `POST /transactions/quick` and `PUT /transactions/{id}` answer the movement plus **`restamped`**, and `DELETE /transactions/{id}` answers its message plus `restamped`: the accounts whose balance the write moved, and, for a movement in a group, the expense and the other lines and movements it rewrote along the way, each with the `updatedAt` it had before and the one it has now, so a device can move the guards of the writes it still has queued on them (T-145, T-146; `docs/modules/sync.md`, _Rows a write rewrote besides its own_). An edit that moves no money, of a movement in no group, answers it empty; so does a replayed create.

...
```json
{
  ...
  "changes": {
    ...
    "categories": [],
    "transactions": [
      { "id": "0195…", "deletedAt": "2026-09-01T10:00:00.000Z", "…": "…" }
    ],
    "budgets": []
  },
  "pagination": {
...
**When `hasMore` is false, the cursor is deliberately 60 seconds behind `serverTime`.** `updatedAt` is stamped by the application server (Mongoose timestamps), not by MongoDB, so two instances with drifted clocks can confirm writes out of order: a row stamped `12:00:00` can become visible _after_ one stamped `12:00:01`. A watermark set to the last row read would skip it forever. Rows inside that window arrive again on the next pull; the client applies by `id` with an upsert, so reprocessing them costs nothing.
  ...
  "operations": [
    {
      ...
      "id": "0195…c1",
      "payload": {
        "body": { "name": "Nequi", "type": "OTHER", "balance": 120000 }
      },
      "dependsOn": [],
      "opVersion": 1
    },
    {
      "opId": "0195…a2",
      "seq": 13,
      "occurredAt": "2026-09-05T10:01:00-05:00",
      "entity": "transaction",
      "action": "create",
      "id": "0195…c2",
      "payload": {
        "body": {
          "type": "EXPENSE",
          "amount": 12500,
          "date": "2026-09-05T10:01:00-05:00",
          "fromAccountId": "0195…c1"
        }
      },
      "dependsOn": ["0195…c1"],
      "opVersion": 1
    },
    {
      "opId": "0195…a3",
      "seq": 14,
      "occurredAt": "2026-09-05T10:02:00-05:00",
      "entity": "budget",
      "action": "setOverride",
      "id": "0195…b7",
      "payload": {
    ...
    },
    {
      "opId": "0195…a4",
      "seq": 15,
      "occurredAt": "2026-09-05T10:03:00-05:00",
      "entity": "sharedExpense",
      "action": "create",
      "id": "0195…e1",
      "payload": {
        "params": { "groupId": "0195…g1" },
        "body": { "transactionId": "0195…c2" }
      },
      "dependsOn": ["0195…g1", "0195…c2"],
      "opVersion": 1
    }
  ]
}
  ...
  "results": [
    {
      ...
      "id": "0195…c1",
      "status": "applied",
      "result": { "id": "0195…c1", "name": "Nequi", "updatedAt": "…", "…": "…" }
    },
    {
      "opId": "0195…a2",
      "seq": 13,
    ...
    },
    {
      ...
      "result": { "categoryId": null, "pendingDetails": true, "…": "…" }
    }
  ]
}
```
...
| ----------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `applied`   | Landed now                                                                                                                                                                                                                                                                                                                                            | `result`: what the route would have answered (absent for `transaction:delete`, whose route answers a message); `warnings` when it landed degraded; `restamped` on the writes that rewrite other rows (below) |
...
**Rows a write rewrote besides its own** (T-145). Every write of the shared layer can rewrite rows it does not answer for: a new amount on a movement restates its expense, and any change to what somebody owes imputes their payments again, which rewrites the `collected` of their other lines and the `countsAsYours` and history of the movements behind them. Each of those rows gets a new `updatedAt`, so a write the device queued on one of them — guarded by the stamp it had read — would meet a `409 STALE_UPDATE` against its own earlier write. The routes that can do this (`POST /transactions`, `POST /transactions/quick`, `PUT`/`DELETE /transactions/{id}`, creating, editing and deleting a group's expense, recording and undoing a payment, adding people to a group, archiving it, writing off and undoing a write-off) answer **`restamped`**: `[{ entity, id, previousUpdatedAt, updatedAt }]`, one per row other than the one the route answers, with the stamp it had before the write and the one it holds after it, read inside the same database transaction. It is empty when nothing else moved, and on a replay by client-minted id, which writes nothing. The device may move a queued guard from `previousUpdatedAt` to `updatedAt`, and only from that stamp: the server vouches that nothing else touched the row between the two. In a batch, `restamped` is **lifted out of `result`** onto the operation's own entry, so `result` stays the row and a `transaction:delete` still answers none. It is also **kept in the operation's record**, so a resent `opId` answered `duplicate` carries it again. And the batch does the same rebase itself: a later operation **in the same batch** on a row an earlier one restamped — applied now or answered from the record — whose `baseUpdatedAt` is a `previousUpdatedAt` of that row, runs against its `updatedAt`. That is what lets a device queue a new amount and then a new split of the same expense and send both at once (the client guards only the first operation of each row in a pass), and what keeps the case working when a batch landed only in part and is resent. A row the write **created** (the movements a payment records) is not in the list: nothing could have been queued on it.

**Accounts are rewritten by every movement** (T-146). A movement's `$inc` on its account's balance moves the account's `updatedAt`, so an archive, a new default or any other account write the device queued after a movement it made offline would meet `409 STALE_UPDATE` against itself. Every write that moves a balance — creating a movement (`POST /transactions`, `POST /transactions/quick`, `transaction:create`, `transaction:quickAdd`), an edit that moves money, a delete, a payment recorded or undone, `Add to my ledger` — names **each account it moved** in `restamped`, archived ones included (a reversal still reaches them), with the stamp the account had before the first `$inc` of the write: the increment reads it in the same atomic operation, and the write reads the accounts' final stamps once at its end (one query by `_id`, inside the database transaction; two more on a new default). An edit that moves no money names none. `POST /accounts/{id}/default` names the account it took the default from, whose `isDefault` it cleared. A create replayed by client-minted id or `Idempotency-Key` answers `restamped` empty: the replay moved no balance, and the stamps of the original are not stored, so a device that lost the first answer keeps the guard it had (the lost-answer case the batch covers with the `opId` record).
...
**A guard on a state that already holds.** Same reasoning again, applied to `baseUpdatedAt`: when the guard no longer matches but the row **already carries what the operation asked for**, the operation answers `duplicate` instead of `conflict` `STALE_UPDATE`. Two things reach that state — the operation's own write landed and its registry row did not (the crash above), and another device made the identical change — and in both the state the operation wanted holds, so there is nothing for the user to decide. The check reads the `current` the refused guard already carries, so it costs no extra query, and it is **conservative**: `update` requires every field the body sent to read that way on the row, `archive` and `restore` look at `archivedAt`, `setDefault` at `isDefault`, `setOverride` and `clearOverride` at the period's override, and a field the row cannot answer for (a budget's `periodStartDate`, which the period view does not carry) fails the check and leaves the conflict standing. A guard that failed because the row holds something **else** is still a `conflict`, with that row in `current`.

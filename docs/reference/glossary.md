...
**Mongoose Model**
MongoDB schema definitions using Mongoose. Located in `src/infrastructure/models/`, one file per collection (`AccountModel.ts`, `TransactionModel.ts`, ...), including infrastructure-only collections with no domain entity (`IdempotencyKeyModel`, `RateLimitModel`, `RefreshSessionModel`, `SharedCounterpartyModel`).

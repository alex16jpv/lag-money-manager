 ...
 *       at a debt account (CARD, OVERDRAFT, LOAN) is never an INCOME — it is a TRANSFER
 *       from wherever it came from, or an ADJUSTMENT when it came from outside the app —
 *       and nothing may leave a LOAN above zero, because a loan cannot be paid more than
 *       it owes. Everything else stays open: spending with a card, a cash advance out of
 *       one, or reconciling any account.
 *
 ...
 *               $ref: '#/components/schemas/Transaction'
 *       400:
 *         description: Validation error. Codes include FUTURE_DATE (date more than 24h in the future), CURRENCY_MISMATCH (transfer between accounts with different currencies), INCOME_ON_DEBT_ACCOUNT (an income landing on a CARD, an OVERDRAFT or a LOAN), LOAN_OVERPAID (a movement that would leave a LOAN above zero), CATEGORY_ARCHIVED, CATEGORY_TYPE_MISMATCH, IDEMPOTENCY_KEY_INVALID (malformed Idempotency-Key header).
 *         content:
 *           application/json:
 *             schema:
 *               $ref: '#/components/schemas/ErrorResponse'
 ...
 *               $ref: '#/components/schemas/Transaction'
 *       400:
 *         description: Validation error. Codes include NO_DEFAULT_ACCOUNT (no account id given and no default account set), FUTURE_DATE, CURRENCY_MISMATCH, INCOME_ON_DEBT_ACCOUNT (a quick income whose default account is a debt one), LOAN_OVERPAID, CATEGORY_ARCHIVED, CATEGORY_TYPE_MISMATCH, IDEMPOTENCY_KEY_INVALID.
 *         content:
 *           application/json:
 *             schema:
 *               $ref: '#/components/schemas/ErrorResponse'
 ...
 *               $ref: '#/components/schemas/Transaction'
 *       400:
 *         description: Validation error. Codes include FUTURE_DATE, CURRENCY_MISMATCH, INCOME_ON_DEBT_ACCOUNT and LOAN_OVERPAID (both checked again whenever the edit moves money), CATEGORY_ARCHIVED (assigning an archived category; keeping the one it already had is allowed), CATEGORY_TYPE_MISMATCH.
 *         content:
 *           application/json:
 *             schema:
 *               $ref: '#/components/schemas/ErrorResponse'
